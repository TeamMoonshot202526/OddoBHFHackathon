...
const NewsFlow = () => {
  ...
  const handleAiSummary = async (event: NewsEvent) => {
    ...
    try {
      ...
      const out = await llmSummarizeEvent(llmApiKey, event);
      patchEvent(event.event_id, { ai_summary: out.summary, ai_key_points: out.key_points, ai_tone: out.tone });
      toast({ title: 'AI summary added' });
    } catch (e: any) {
        ...
        <SheetContent className="w-full sm:max-w-xl p-0">
          <ScrollArea className="h-full">
            <div className="p-6">
              ...
              <Button size="sm" variant="secondary" onClick={() => selectedEvent && handleAiSummary(selectedEvent)} className="ml-auto">
                <BookOpen className="w-4 h-4 mr-2" />AI Summary
              </Button>
e>
                {selectedEvent && (
